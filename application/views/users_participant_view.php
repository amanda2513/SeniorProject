	...
	<script type="text/javascript">
		function confirmModal(id){
			bootbox.dialog("Are you sure you want to delete this?",[{
				...
				"callback": function(){
					window.location.href='<?php echo base_url()."manage_users/delete/participant/";?>'+id;
				}
			}, {
				"label": "Cancel",
	...
	<div class="hero-unit wsu_hero_unit">
		...
		<form class="form-search"  method="post" action='<?php $id=1; echo base_url()."manage_users/search_users_participant_view/$id"; ?>'>
			<div class="input-append">
				<input type="text" name="search_participant" id="search_participant" class="input-large search-query" placeholder="Participant's Last Name">
				<button class="btn wsu_btn" type="submit" id="search"><i class="icon-search"></i> Search</button>
			</div>
			<a class="btn wsu_btn" id="clear" href="<?php echo base_url()."manage_users/participant";?>">Clear</a>
		</form>

		

		<ul class="nav nav-tabs">
			<li class="active"><a href='<?php echo base_url()."manage_users/participant"?>'>Participants</a></li>
			...
			<a class="btn wsu_btn pull-right enable_buttons" href="<?php echo base_url()."manage_users/add?type=".urlencode("participant");?>" id="btn_add_participant"><i class="icon-plus"></i> Add Participant</a>
			<a class="btn wsu_btn pull-right enable_buttons" href="<?php echo base_url()."manage_users/disable_all/participant";?>" id="disable_all_participant"><i class="icon-lock"></i> Disable All Participants</a>
			<a class="btn wsu_btn pull-right enable_buttons" href="<?php echo base_url()."manage_users/enable_all/participant";?>" id="enable_all_participant"><i class="icon-lock"></i> Enable All Participants</a>
		</ul>
						...
						foreach($participant as $row){
							...
							'<tr>
								...
								if($row->status=="Enabled"){
									...
									<td>' . $row->status .'</td>
									<td>
										<a class="btn wsu_btn wsu_tooltip" rel="tooltip" title="Disable Participant Access" href="'.base_url()."manage_users/change_user_status/participant/Disabled/".$row->id.'"><i class="icon-lock"></i></a>';
								}
								else{
									echo'
									<td>' . $row->status .' <i class="icon-lock"></i></td>
									<td>
										<a class="btn wsu_btn wsu_tooltip" rel="tooltip" title="Enable Participant Access" href="'.base_url()."manage_users/change_user_status/participant/Enabled/".$row->id.'"><i class="icon-lock"></i></a>';
								}
								echo' 
									<a class="btn wsu_btn wsu_tooltip" rel="tooltip" title="Edit Participant Info" href="'.base_url()."manage_users/edit/".$row->usertype."/".$username.'"><i class="icon-pencil"></i></a>
									<button class="btn wsu_btn wsu_tooltip" rel="tooltip" title="Delete Participant" onClick="confirmModal('."'".$row->id ."'".')"><i class="icon-trash"></i></button>
								</td>
